...
/* ── Helpers ── */
const imgSrc = (img) =>
  !img ? null : img.startsWith('http') ? img : `http://localhost:8000${img}`;
...
export default function RestaurantDetail() {
  ...
  const handleAddToCart = async (food) => {
    ...
    } catch { setCartMsg('Failed to add.'); setTimeout(() => setCartMsg(''), 2500); }
  };
  ...
  const heroImg = imgSrc(restaurant?.image);
  const isOpen = () => {
