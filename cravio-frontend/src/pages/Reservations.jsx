...
export default function Reservations() {
  ...
  useEffect(() => {
    ...
    // Auto-open form if any URL param is present
    if (restaurantFromUrl || searchParams.get('date') || searchParams.get('time') || searchParams.get('guests')) {
      setShowForm(true);
    }
  }, []);
  ...
  const handleResendOtp = async () => {
    if (resendCooldown > 0) return;
    try {
      await api.post(`/reservations/${pendingReservation.id}/resend-otp/`);
      setResendCooldown(60);
      setOtpError('');
      ...
      <div className="container-cravio">
        ...
        {pendingReservation && (
          <div style={{ background: 'white', borderRadius: 16, border: '2px solid var(--olive)', padding: '36px 32px', marginBottom: 28, maxWidth: 480, margin: '0 auto 28px' }}>
            ...
            {/* Reservation summary */}
            <div style={{ background: 'var(--cream)', borderRadius: 10, padding: '12px 16px', marginBottom: 24, fontSize: '0.85rem', color: 'var(--dark-soft)', display: 'flex', gap: 20, justifyContent: 'center', flexWrap: 'wrap' }}>
        ...
        {showForm && !pendingReservation && (
          <div style={{ background: 'white', borderRadius: 14, border: '1px solid var(--border)', padding: '28px', marginBottom: 28 }}>
            ...
            <form onSubmit={handleSubmit} className="form-cravio" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                {form.restaurant && restaurants.find(r => String(r.id) === String(form.restaurant)) ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ flex: 1, padding: '10px 14px', borderRadius: 8, border: '1.5px solid var(--olive)', background: 'var(--olive-pale)', color: 'var(--olive)', fontWeight: 600, fontSize: '0.92rem' }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <button type="submit" className="btn-olive" style={{ padding: '11px 28px' }} disabled={submitting}>
