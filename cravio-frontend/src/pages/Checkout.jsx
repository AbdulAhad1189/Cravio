...
export default function Checkout() {
  ...
  // Delivery location state (owner-style)
  ...
  const handlePincodeChange = async (e) => {
    ...
    if (pin.length === 6) {
      ...
      if (result) {
        ...
        // Save to store so navbar + home page update too
  ...
  const handleUseCurrentLocation = () => {
    detectCurrentLocation(
      (loc) => {
        ...
        // Already saved to store by detectCurrentLocation
      },
      (err) => alert(err),
      setGeoLoading
    );
  ...
  const handleSubmit = async (e) => {
    ...
    if (!fullAddress.trim()) { alert('Please enter your delivery address.'); return; }
    setLoading(true);
    try {
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ background: 'white', borderRadius: '14px', border: '1px solid var(--border)', padding: '28px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: 10 }}>
                <h3 style={{ fontWeight: 700, fontSize: '1.05rem', display: 'flex', alignItems: 'center', gap: 8, margin: 0 }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--olive)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                  Delivery Address
                </h3>
              ...
              <div className="form-cravio" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                {/* Pincode row — autofills city + state */}
                <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr 1fr', gap: 14, alignItems: 'end' }}>
                  <div>
                    <label>Pincode *</label>
                    <div style={{ position: 'relative' }}>
                      ...
                      />
                      {pincodeLoading && (
                        <span style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', fontSize: '0.7rem', color: 'var(--olive)', whiteSpace: 'nowrap' }}>
                          fetching…
                        </span>
                      )}
                    </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={city}
                      onChange={e => { setCity(e.target.value); saveLocStore({ city: e.target.value }); }}
                      required
                    />
                  </div>
                  <div>
                    <label>State *</label>
                    <select value={state} onChange={e => { setState(e.target.value); saveLocStore({ state: e.target.value }); }} required>
                ...
                {(streetAddress || city || state) && (
                  <div style={{ background: 'var(--cream)', border: '1px dashed var(--border)', borderRadius: 8, padding: '10px 14px', fontSize: '0.82rem', color: 'var(--text-muted)', display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--olive)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginTop: 1, flexShrink: 0 }}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                    <span>{[streetAddress, city, state, pincode].filter(Boolean).join(', ')}</span>
                  </div>
                )}
